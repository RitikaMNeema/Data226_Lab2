...
price_changes AS (
    SELECT
        ...
        date_ts,
        close,
        close - LAG(close, 1) OVER (PARTITION BY symbol ORDER BY date_ts) AS price_change
    FROM stock_data
),
...
SELECT
    ...
    -- RSI momentum
    CASE 
        WHEN rsi_14 > LAG(rsi_14, 1) OVER (PARTITION BY symbol ORDER BY date_ts) 
        THEN 'INCREASING'
        WHEN rsi_14 < LAG(rsi_14, 1) OVER (PARTITION BY symbol ORDER BY date_ts) 
        THEN 'DECREASING'
        ELSE 'FLAT'
